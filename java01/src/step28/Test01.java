...
public class Test01 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // => com.mysq.jdbc.Driver ---> JDBC 드라이버 관리자에게 Connection 구현체를 리턴한다.
    // => 즉 com.mysq.jdbc.ConnectionImpl 객체를 리턴한다.
    ...
    //=> 안 닫는다면? 
    //   일정 시간동안 DBMS 서버 메모리에 연결 정보가 남아 있는다.
    //=> 연결정보가 좀 남아있다면? 
    //   DBMS는 연결 수를 관리한다.
    //   연결 수가 최대 값을 초과하면 더이상 연결을 허락하지 않는다.
    //   그래서 항상 커넥션을 다 사용한 다음에는 즉시 끊는 것이 중요한다.
    //=> 물론 안 끊는다면? 
